...
ts_rain <- ts(BRA_complete$Rainfall, start = c(1901-01-01), frequency = 12)
ts_temp <- ts(BRA_complete$Temperature_C, start = c(1901-01-01), frequency = 12)
...
ts_rain_decomposed <- stl(ts_rain, s.window = "periodic")
plot(ts_rain_decomposed)

ts_temp_decomposed <- stl(ts_temp, s.window = "periodic")
plot(ts_temp_decomposed)
...
rain_dec_components <- as.data.frame(ts_rain_decomposed$time.series[,1:3])

temp_dec_components <- as.data.frame(ts_temp_decomposed$time.series[,1:3])


Temp_Rain_dec_comp <- cbind(BRA_complete$Date, rain_dec_components, temp_dec_components)




# Exercise 10: Visualize how the trend maps onto the data for both series
ggplot (rain_dec_components, aes(x= Date)) +
  geom_line(aes(y= )) plot(rain_dec_components)

ggplot(Temp_Rain_dec_comp, aes(x=Date)) +
  geom_line(aes(y=))
