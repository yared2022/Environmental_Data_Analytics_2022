...
#Exercise 3: Run a linear regression PM2.5 by Ozone. Find the p-value and R-squared value (These are measures of goodness of fit).
PM2.5_reg <- lm(data=EPAair, PM2.5 ~ Ozone)
summary(PM2.5_reg)

# The p-value is less than 0.05 (<2.2e-16) and the R^2 is 0.1316. 
...
library(corrplot)
# Start with by creating a subset of the selected variables in a separate
# EPAair.subset <- 
PM2.5_pre. <- EPAair %>% 
  select(PM2.5, meanLat, meanLng, Ozone) %>% 
  na.omit()

PM2.5_pre.Corr <- cor(PM2.5_pre.)
corrplot(PM2.5_pre.Corr, method = "ellipse")
corrplot.mixed(PM2.5_pre.Corr , upper = "ellipse")
...
PM2.5_AIC <- lm(data = EPAair, PM2.5 ~ meanLat + Ozone + meanLng) 
step(PM2.5_AIC)
