...
head(NTL.phys.data_1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.

NTL.phys.data_TuLa <- filter(NTL.phys.data, lakename =="Tuesday Lake" & year4 %in% c(1990,1999))

tail(NTL.phys.data_TuLa)
...
NTL.phys.data.Ex3 <- NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999)) %>%
  filter(sampledate(month)==7)
...
NTL.phys.data.Ex3 %>%
filter(depth==0)%>%
filter(!is.not(temprature_C)) %>%
  Summarize(mean = mean(temprature_C))
